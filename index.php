...
if (!empty($_POST)) {
  if (isset($_POST['Submit'])) {
    ...
    if ($resultUser->num_rows > 0) {
      ...
      if ($input_username === 'admin' && $input_password === 'admin1') {
        $_SESSION['is_admin'] = true;
        header("Location: adminCode/admin_menu.php"); // redirect to admin page
        exit(); // make sure to exit to prevent further execution of code
      } else {
        // regular user login
        header("Location: studentsCode/view_students.php"); // redirect to regular user page page
        exit(); // make sure to exit to prevent further execution of code
      }
    } else {
